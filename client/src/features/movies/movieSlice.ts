...
export const fetchMovies = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("movies/fetchMovies", async (page, thunkAPI) => {
  try {
...
export const fetchMovieBySearch = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("movies/fetchMovieBySearch", async (search, thunkAPI) => {
  try {
    const response = await axios.get(
      `${BASE_URL}/search/movie?query=${search}&api_key=${apiKey}&language=en-US&page=1&include_adult=false`
    );
