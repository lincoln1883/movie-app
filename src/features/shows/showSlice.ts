...
export const fetchShowBySearch = createAsyncThunk<Show[], string, {rejectValue: string}>("shows/fetchShowBySearch",async (search, thunkAPI) => {
	try {
		const response = await axios.get(`${BASE_URL}/search/tv?query=${search}&api_key=${apiKey}&language=en-US&page=1&include_adult=false`);
...
const showSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchShowBySearch.fulfilled, (state, action: PayloadAction<Show[]>) => {
			state.status = 'success';
			state.shows = action.payload;
		});
