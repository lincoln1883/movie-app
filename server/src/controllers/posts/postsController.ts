...
export const getPosts = async (req: Request, res: Response) => {
  try {
    const posts = await Post.find().populate('likes').populate('comments').sort({ createdAt: -1 });
    if (!posts) {
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const updatedPost = await Post.findByIdAndUpdate(id, { title, overview, poster_path, release_date, vote_average,reviews }, { new: true });
